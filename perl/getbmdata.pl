...
while(<LF>) {
   ...
   if ("$Option" == 1) {
      if (/One checkout/) {
          ...
          print "$Time\n";
          if ($Time > $coth) {
              &Notify("clearcase-admins-list\@broadcom.com","CC_MON_WARNING: Warning, ClearCase checkout command exceeded threadshold on $Server\n","Checkout time on $Server is $Time.\n\n\n\n");
          }
      }
      if (/One uncheckout/) {
          s/ +//g; #Remove blanks
   ...
   if ("$Option" == 2) {
      if (/One checkin/) {
          ...
          print "$Time\n";
          if ($Time > $cith) {
              &Notify("clearcase-admins-list\@broadcom.com","CC_MON_WARNING: Warning, ClearCase checkin command exceeded threadshold on $Server\n","Checkin time on $Server is $Time.\n\n\n\n");
          }
      }
      if (/Ten checkin/) {
          s/ +//g; #Remove blanks
