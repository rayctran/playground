...
$report{dir}="$gnats{gnats_root}/www/htdocs/reports/$gnats{database}/weekly/${year}/${month}";
$report{file}="$report{dir}/$gnats{top_level_cat}_5weeks_rpt_${date}.html";
$report{web_link}="http\:\/\/gnats-irva-3.broadcom.com\/reports\/$gnats{database}\/weekly\/${year}\/${month}\/$gnats{top_level_cat}_5weeks_rpt_${date}.html";
...
sub main {
    ...
    while(<DBFILE>) {
            ...
            if ( $gnats{database} =~ /$db{name}/ ) {
                ...
                $found = 1;
                if (! -d "$report{dir}") {
                    print "Creating dir $report{dir}\n";
                    system("mkdir -p $report{dir}; chmod 776 $report{dir}");
                }
                last;
            }
    }
